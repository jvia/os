...
static int slurping[ANTHILLS]= { 0,0,0,0 };// how many aardvarks are slurping
...
static void trace_state() { 
    if (trace) 
    printf(
	"%09.6f (trace) Ants: %d %d %d %d Slurping: %d %d %d %d, Swallowing: %d, Sulking: %d\n",
	elapsed(), 
	ants_left[0], ants_left[1], ants_left[2], ants_left[3], 
	slurping[0],  slurping[1],  slurping[2], slurping[3], 
	swallowing, sulking); 
} 
